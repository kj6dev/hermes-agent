...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_movie_monitor_callback(
        ...
        *,
        ...
        # Look up Letterboxd slug for x-callback-url.
        x_callback_url = self._movie_monitor_x_callback_url(track_id)

        await query.answer(text="Marked in alert ✓")
        try:
            await query.edit_message_reply_markup(reply_markup=keyboard)
        except AttributeError:
            await query.edit_message_text(
        ...
        if bot and query_chat_id is not None:
            text = f"✅ Marked *{title}* in this Telegram alert."
            reply_markup = None
            if x_callback_url:
                text += "\nTap below to add it to your Letterboxd purchases list."
                reply_markup = InlineKeyboardMarkup([
                    [InlineKeyboardButton("Open Letterboxd", url=x_callback_url)]
                ])
            try:
                ...
                logger.debug("Failed to send movie monitor ack: %s", exc)

    @staticmethod
    def _movie_monitor_x_callback_url(track_id: str) -> Optional[str]:
        """Build a Letterboxd x-callback-url for adding a film to the purchases list."""
        try:
            from hermes_constants import get_hermes_home

            config_path = get_hermes_home() / "agents" / "movies" / "config.json"
            mapping_path = get_hermes_home() / "agents" / "movies" / "track_to_slug.json"

            if not mapping_path.exists() or not config_path.exists():
                return None

            config = json.loads(config_path.read_text(encoding="utf-8"))
            if not config.get("live_letterboxd_writes"):
                return None

            mapping = json.loads(mapping_path.read_text(encoding="utf-8"))
            slug = mapping.get(track_id)
            if not slug:
                return None

            owned_list = config.get("owned_list_slug", "purchases")
            app_uri = f"letterboxd://film/{slug}/add-to-list/{owned_list}"

            import base64

            encoded = base64.urlsafe_b64encode(app_uri.encode("utf-8")).decode("ascii").rstrip("=")
            return f"https://go.kj6.dev/raw/{encoded}"
        except Exception as exc:
            logger.debug("Failed to build x-callback-url: %s", exc)
            return None
